...
import { BookOpen, Copy, Plus, Search } from 'lucide-vue-next'
...
<template>
  <div class="page-container">
    ...
    <div class="card-grid card-grid--3"><article v-for="item in filtered" :key="item.id" class="resource-card"><div><span class="tag">{{ item.resource_type }}</span><span v-if="item.visibility === 'system'" class="resource-system">MLab 精选</span></div><h3>{{ item.title }}</h3><p>{{ item.description }}</p><div class="resource-preview">{{ item.content }}</div><div class="card-footer"><div><span v-for="tag in item.tags" :key="tag" class="tag">{{ tag }}</span></div><button class="icon-button" title="复制内容" @click="copy(item)"><Copy :size="17" /></button></div></article></div>
  </div>
</template>
